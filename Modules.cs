    ...
    internal class Modules
    {
        ...
        public static void cookiechecker()
        {
            ...
            try
            {
                ...
                File.AppendAllText("Output.csv", $"USERID, USERNAME, ROBUX, RAP, PREMIUM, CREDIT, PENDINGROBUX, DATE OF CREATION, EMAILVERIFIED" + Environment.NewLine);

                string file = new StreamReader("cookies.txt").ReadToEnd();
                string[] lines = file.Split('\n');
                int CountCookies = lines.GetLength(0);
                    ...
                    using (StreamReader reader = new StreamReader("Cookies.txt"))
                    {
                        ...
                        while ((line = reader.ReadLine()) != null)
                        {
                            ...
                            if(api.Contains("good"))
                            {
                                ...
                                File.AppendAllText("Output.csv", $"{USERID},{USERNAME},{ROBUX},{RAP},{PREMIUM},{CREDIT},{PENDINGROBUX},{DOC},{EMAILVERIFIED}, {line}" + Environment.NewLine);
                                Console.WriteLine("");
                            }
                            else
                            {
        ...
        public static void settings()
        {
            try
            {
                ...
                Console.WriteLine();
                //To add next update
                //Change directory through settings
                //Pick which format to use .csv or .txt
                //Use proxies (unnecessary)
                ...
                Console.ResetColor();
                Console.ForegroundColor = ConsoleColor.Cyan;
                Console.Write("[1] [DISABLED] CRYSTAL SETTING: ");
                Console.ResetColor();
                Console.Write("Set a new directory");
